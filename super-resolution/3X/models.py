...
class CC_Module(nn.Module):
    ...
    def __init__(self, scale):
        ...
        self.d_conv2 = nn.ConvTranspose2d(in_channels=35, out_channels=3*scale*scale, kernel_size=3, stride=1, padding=1)
        self.d_bn2 = nn.BatchNorm2d(num_features=3*scale*scale)
        self.d_relu2 = nn.PReLU(3*scale*scale)

        self.pixel_shuffle = nn.PixelShuffle(scale)
        self.adjust_conv = nn.Conv2d(in_channels=3, out_channels=3, kernel_size=3, stride=1, padding=(1,0))
